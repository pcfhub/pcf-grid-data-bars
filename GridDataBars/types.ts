// The Power Apps grid customizer contract, vendored.
//
// Source: microsoft/PowerApps-Samples, master branch, taken 2026-08-23 —
// component-framework/resources/GridCustomizerControlTemplate/PAGridCustomizer/types.ts
// Documented at
// https://learn.microsoft.com/en-us/power-apps/developer/component-framework/customize-editable-grid-control
//
// Vendored rather than installed, because there is nothing to install: this is
// a file inside a samples repository that authors are told to clone, not a
// published package, and `@types/powerapps-component-framework` does not carry
// it — that package does not even declare `factory.fireEvent`, which is how the
// payload below reaches the host. The provenance comment above is the whole
// mitigation for that: when a control starts handing us a shape this file does
// not describe, the fix is to re-take it from the same path and diff.
//
// Kept verbatim apart from this header and the import line — no fields added,
// none removed, none tidied. A local improvement here would be a silent
// divergence from the interface third-party controls are compiled against.

import type { ComponentClass, MouseEvent, ReactElement } from 'react';
...
export interface GridCustomizer {
    ...
    GetHeaderRenderer?(params: GetHeaderParams): ReactElement;
    /** Returns the cell renderer react element */
    GetCellRenderer?(params: GetRendererParams): ReactElement;
    /** Return renderer for a row in a loading state */
    GetLoadingRowRenderer?(): ReactElement;
    /** Returns the cell editor react element */
    GetCellEditor?(params: GetEditorParams): ReactElement;
